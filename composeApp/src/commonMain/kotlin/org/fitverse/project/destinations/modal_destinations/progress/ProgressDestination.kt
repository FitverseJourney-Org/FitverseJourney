...
// 1. Extraímos os dados para um objeto de Mock para limpar a UI
private object ProgressMocks {
    val exercises = listOf(
        Exercise("1", "Supino Reto (Barra)", "Peito", "Treino A"),
        Exercise("2", "Levantamento Terra", "Costas", "Treino B"),
        Exercise("3", "Agachamento Livre", "Pernas", "Treino C"),
        Exercise("4", "Desenvolvimento Halteres", "Ombros", "Treino D")
    )

    val currentProgression = listOf(
        ...
        LoadProgressionPoint(DateTimeManager.create(20, 6, 2026), 85.0, 102.0)
    )

    val previousProgression = listOf(
...
fun ProgressDestination(toBack: () -> Unit) {
    // Agora a função é pura e foca apenas em passar os dados adiante.
    // Em uma arquitetura real, esses dados viriam de um ViewModel via collectAsState()

    ProgressScreen(
        exercises = ProgressMocks.exercises,
        currentProgression = ProgressMocks.currentProgression,
        monthbeforeProgression = ProgressMocks.previousProgression,
        onBack = toBack
    )
}
