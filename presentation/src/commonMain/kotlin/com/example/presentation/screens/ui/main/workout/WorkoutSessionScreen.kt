...
fun WorkoutSessionScreen(
    ...
    LaunchedEffect(isRunning, isResting, currentIndex, currentSet) {
        ...
        scoped.launch {
            if (current.type == ExerciseType.TIMED) {
                // countdown
    ...
    LaunchedEffect(key1 = isResting) {
        ...
        if (next != null && next.type == ExerciseType.TIMED) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp), colors = CardDefaults.cardColors(containerColor = SurfaceGreen)) {
                Column(modifier = Modifier.padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Button(onClick = {
                            ...
                            if (isRunning && current.type == ExerciseType.TIMED && timerSeconds <= 0) {
                                timerSeconds = current.durationSeconds
                            }
                        }) {
...
@Composable
fun SmallExercisePreview(ex: Exercise, onClick: () -> Unit) {
    Card(
        modifier = Modifier
        ...
        Column(modifier = Modifier.padding(8.dp), horizontalAlignment = Alignment.Start) {
            ...
            Text(
                when (ex.type) {
                    ExerciseType.TIMED -> "${ex.durationSeconds}s"
                    ExerciseType.REPS -> "${ex.reps} reps"
