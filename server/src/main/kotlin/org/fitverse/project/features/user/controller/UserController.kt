...
class UserController(
    ...
    suspend fun get(call: ApplicationCall) {
        ...
        if (result.isSuccess) {
            call.respondSuccess(data = result.getOrNull())
        } else {
            call.respondError(
                code = "NOT_FOUND",
    ...
    suspend fun create(call: ApplicationCall) {
        ...
        if (result.isSuccess) {
            call.respondSuccess(data = result.getOrNull(), status = HttpStatusCode.Created)
        } else {
            call.respondError(
                code = "CREATE_ERROR",
    ...
    suspend fun delete(call: ApplicationCall) {
        ...
        if (result.isSuccess) {
            call.respond(HttpStatusCode.NoContent) // sem body, sem problema de serialização
        } else {
            call.respondError(
                code = "DELETE_ERROR",
